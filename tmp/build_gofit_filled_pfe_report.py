...
def professional_test_rows(n):
    ...
    rows = {
        3: [
            ["S1-T01", "Authentication", "Valid user signs in", "Test account exists", "Open app; enter credentials; submit", "Protected navigator opens", "Not documented in report", "To be validated", evidence],
            ["S1-T02", "Onboarding", "New user saves profile", "Authenticated user without onboarding", "Fill goals, units, preferences; save", "Profile is stored and onboarding flag updates", "Not documented in report", "To be validated", evidence],
            ["S1-T03", "Workout session", "User completes workout", "Workout template exists", "Start workout; complete sets; finish summary", "Session is saved in workout history", "Not documented in report", "To be validated", evidence],
            ["S1-T04", "Rest timer", "Timer controls behave correctly", "Active workout session", "Start, pause, resume, warning, complete", "Timer state and feedback remain consistent", "Not documented in report", "Manual validation required", evidence],
        ],
        4: [
            ["S2-T01", "Exercise library", "Search and open exercise", "Exercise data exists", "Search; filter; open detail", "Correct exercise detail is displayed", "Not documented in report", "To be validated", evidence],
            ["S2-T02", "Progress dashboard", "Display charts and empty state", "User with/without history", "Open progress; switch metric and period", "Charts or empty state render correctly", "Not documented in report", "To be validated", evidence],
            ...
            ["S2-T04", "Notifications", "Configure reminder permission", "Device/emulator supports notifications", "Request permission; save reminder preference", "Preference persists and no crash occurs", "Not documented in report", "Manual validation required", evidence],
            ["S2-T05", "Nutrition/saved meals", "Create or reuse saved meal", "Authenticated user", "Create saved meal; edit item; reopen list", "Meal entry persists in history", "Not documented in report", "To be validated", evidence],
        ],
        5: [
            ["S3-T01", "Coach onboarding", "Coach submits profile", "Coach account exists", "Fill profile; upload/support documents; submit", "Coach appears in pending/review state", "Not documented in report", "To be validated", evidence],
            ["S3-T02", "Marketplace booking", "Client books coach", "Coach and slot/pack exist", "Open marketplace; select coach; confirm booking", "Booking is visible to client and coach", "Not documented in report", "To be validated", evidence],
            ["S3-T03", "Chat", "Client and coach exchange messages", "Conversation exists", "Send message from one side; open other side", "Message appears in conversation", "Not documented in report", "Manual validation required", evidence],
            ["S3-T04", "Admin access", "Non-admin blocked from admin panel", "Non-admin account and admin route exist", "Attempt to open admin route", "Access is denied or redirected", "Not documented in report", "To be validated", evidence],
...
def introduction(doc):
    ...
    p(doc, "The main objective of the internship was to design, implement, and document a complete software foundation for GoFit. The work included requirements analysis, Agile planning, database modeling, mobile and web development, backend integration, security considerations, testing, and deployment preparation.")
...
def chapter6(doc):
    ...
    add_basic_table(
        ...
        ["Test ID", "Feature", "Scenario", "Preconditions", "Steps", "Expected", "Actual", "Status", "Evidence"],
        [
            ["F-T01", "End-to-end mobile flow", "Client signs in and completes core workout flow", "Test user and workout data available", "Sign in; open workout; complete session; view history", "Session appears in history and progress data", "Not documented in report", "To be validated", "TODO: attach screenshots or device recording"],
            ["F-T02", "Database/RLS", "User cannot access another user's data", "Two test accounts and RLS policies enabled", "Query scoped data from each account", "Only authorized rows are returned", "Not documented in report", "To be validated", "TODO: attach SQL or Supabase policy test output"],
            ["F-T03", "Admin panel", "Non-admin is blocked and admin can manage data", "Admin and non-admin accounts exist", "Open admin routes with both accounts", "Admin allowed; non-admin denied", "Not documented in report", "To be validated", "TODO: attach browser screenshots"],
            ...
            ["F-T06", "Deployment", "Mobile/admin/backend/automation deployment readiness", "Production env variables and build config prepared", "Run EAS build, admin deployment, Supabase checks, and n8n workflow import", "Builds and hosted services are reachable; workflows are imported and scheduled", "Not documented in report", "Manual validation required", "TODO: attach build/deployment output"],
        ],
        [0.48, 0.72, 0.88, 0.85, 1.05, 0.95, 0.9, 0.76, 0.91],
    )
    doc.add_page_break()
...
def final_pages(doc):
    ...
    p(doc, "The GoFit graduation internship project produced a complete software foundation for a modern fitness platform. It combines a mobile application for clients and coaches, a web administration panel, and a Supabase backend. The project covers authentication, onboarding, profile management, workout planning, exercise library, active workout sessions, rest timer, progress tracking, nutrition-related flows, coach marketplace features, bookings, chat, programs, notifications, analytics, administration, and deployment preparation.")
