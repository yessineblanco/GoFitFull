...
todos:
  ...
  - id: ai-recommendations
    ...
    status: completed
  - id: progress-photos
    content: "Part 5: Progress Photos v1 -- private DB/bucket/RLS + mobile signed-URL photo journal, timeline grid, detail view, add/delete flow"
    status: completed
  - id: wearables
  ...
  - id: coach-ui-polish
    ...
    status: completed
  - id: mobile-plan-session-nav-calendar
    content: "Mobile: Plan-started workout exit (beforeRemove + Library reset + WorkoutsMain initial false) + mark workout_plans completed on session finish + thread planId through WorkoutDetail"
    status: completed
  - id: marketplace-elite-trainers-rls-mobile
    content: "Marketplace: RLS allows all approved coaches + mobile Elite Trainers real data (service normalize, TopTrainers focus refresh, Home pull refresh, i18n); migration applied via Supabase MCP"
    status: completed
isProject: false
---
...
- **Part 6 - Wearables database v1 (2026-04-27)** - Migration `database/migrations/create_health_data_wearables_v1.sql`: `public.health_data` (id, user_id → auth.users, date, steps, active_calories, source `health_connect`|`manual`, synced_at, timestamps, **UNIQUE (user_id, date, source)**), RLS SELECT/INSERT/UPDATE/DELETE own rows with WITH CHECK on insert/update, `GRANT` to `authenticated`, indexes on `(user_id, date desc)` and `(user_id, source, date desc)`, trigger `set_health_data_updated_at` → `public.handle_updated_at()`. **Applied to `rdozeaacwaisgkpxjycn` via Supabase MCP** (`apply_migration`, name `create_health_data_wearables_v1`). **Next:** mobile Health Connect / Apple Health sync and upsert into this table.
- **Part 6 - Wearables Android Health Connect mobile v1 (2026-04-27)** - Shipped Android-first daily activity sync on top of `health_data`. Added `react-native-health-connect` + `expo-health-connect` and the Expo config plugin; `GoFitMobile/src/services/healthSync.ts` checks/request Health Connect permissions, aggregates daily `Steps` and `ActiveCaloriesBurned` for today plus previous 6 days, and upserts with `onConflict: 'user_id,date,source'`; `GoFitMobile/src/store/healthStore.ts` holds connection/sync state with a 15-minute foreground-sync cooldown. Added `HealthWidget` on Home, a Progress 7-day health activity card, and `HealthSyncScreen` under Profile settings. iOS HealthKit, true background sync, sleep/heart/distance metrics, and BI/admin health analytics remain deferred. Verification: `npm run type-check` and `git diff --check` pass; Android device/dev-build testing still required because Health Connect is native and unavailable in Expo Go.
- **Mobile TypeScript baseline cleanup (2026-04-27)** - Cleared the existing `GoFitMobile` `npm run type-check` blockers without a broad refactor. Fixes covered route type drift for Profile `VideoCall`, unsupported `sceneContainerStyle` on the coach tab navigator, missing `ActivityIndicator` / theme background constants, marketplace RPC result typing, Zustand deep-link store self-reference inference, and notification inbox deep links routed through the existing Home/Profile tab stacks. Verification: `npm run type-check` passes and `git diff --check` passes.
- **Mobile calendar active-session resume affordance (2026-04-25)** - Added a small calendar/plan follow-up so a planned workout card shows **IN PROGRESS** and the primary action says **RESUME** when the selected day already has an incomplete `workout_sessions` row for the same workout. `GoFitMobile/src/services/workoutSessions.ts` now preserves `workout_id` in normalized session rows, `GoFitMobile/src/types/session.ts` exposes it on `WorkoutSession`, and `GoFitMobile/src/screens/plan/MyWorkouts.tsx` refreshes plans/sessions on focus, filters incomplete rows out of completed-session cards, and passes `sessionId` into `Library -> WorkoutSession` for the matching active session instead of starting a new session. No database migration or demo data needed.
- **Marketplace Elite Trainers — RLS + mobile data (2026-04-24)** - **Problem:** Home “Elite Trainers” (`TopTrainers`) uses `marketplaceService.getTopCoaches` → `coach_profiles` with `status = 'approved'`, but RLS only allowed `SELECT` when `is_verified = true AND status = 'approved'`, so **approved coaches without PRO verification were invisible** to clients; cards showed empty/wrong data. **Database:** Added migration `database/migrations/marketplace_coach_profiles_select_approved.sql` (drops `Anyone can view verified coach profiles`, creates `Public can view approved coach profiles for marketplace` with `USING (status = 'approved')`). **Applied to Supabase project `rdozeaacwaisgkpxjycn` via Cursor Supabase MCP** (`apply_migration`, name `marketplace_coach_profiles_select_approved`). Baseline schema aligned in `database/schema/create_coach_marketplace_tables.sql` for fresh installs. **Mobile:** `GoFitMobile/src/services/marketplace.ts` — normalize `average_rating`, `total_reviews`, `total_sessions`; trim `display_name`; same numeric normalization in `getCoachDetail`. `GoFitMobile/src/components/home/TopTrainers.tsx` — `useFocusEffect` refetch; avatar prefers `profile_picture_url` then `user_profile_picture`; name fallback `marketplace.unknownCoach`; rating shows **New** / `noReviewsYet` when `total_reviews === 0` instead of fake `0.0`. `GoFitMobile/src/screens/home/HomeScreen.tsx` — pull-to-refresh runs `loadTopCoaches()` with sessions `fetch()`. **i18n:** `marketplace.noReviewsYet` in `GoFitMobile/src/i18n/locales/en.json` and `fr.json`. PRO badge still driven by `is_verified`.
- **Marketplace coach display names & profile photos — user_profiles RLS + schema drift (2026-04-24)** - **Problem:** After coach_profiles became readable, the app still joined `user_profiles` for `display_name` / `profile_picture_url`, but RLS on `user_profiles` only allows each user to read **their own** row (`fix_admin_rls_policies.sql`), so the batch `.in('id', userIds)` returned **no rows** — Elite Trainers showed generic “Coach”, initials, and no avatars. Some DBs also lacked a `user_profiles.display_name` column (RPC first apply failed with `column up.display_name does not exist`). **Fix:** Migration `database/migrations/marketplace_coaches_public_directory_rpc.sql` — `ADD COLUMN IF NOT EXISTS` for `display_name` and `profile_picture_url` on `user_profiles`, backfill `display_name` from `auth.users` metadata where empty; **`list_approved_coaches_for_marketplace()`** and **`get_coach_profile_identity(uuid)`** as `SECURITY DEFINER` with `SET search_path = public`, joining `coach_profiles`, `user_profiles`, and **`auth.users`**, returning `COALESCE(trimmed user_profiles.display_name, raw_user_meta_data display_name/full_name, email local-part)` and `COALESCE(user_profiles.profile_picture_url, auth avatar_url)`; `GRANT EXECUTE` to `anon` and `authenticated`. **Applied via Supabase MCP** to `rdozeaacwaisgkpxjycn` (`apply_migration`, name `marketplace_coaches_directory_v1`). **Mobile:** `getApprovedCoaches` now calls `.rpc('list_approved_coaches_for_marketplace')` and applies filters/sort in JS; `getCoachDetail` uses `.rpc('get_coach_profile_identity', { p_user_id })` instead of direct `user_profiles` select.
...
- **Mobile plan-started workout exit + calendar plan completion sync (2026-04-24)** - **Navigation bug:** Sessions started from the plan/calendar used `returnTo: 'Plan'` and `beforeRemove` on `WorkoutSessionScreen` called `preventDefault`, then on confirm dispatched `CommonActions.reset` to `LibraryMain`; that second navigation re-fired `beforeRemove` without an allow flag, so the listener could block its own reset and leave `Library -> WorkoutSession` mounted. `CustomTabBar` correctly hides when the active stack top is `WorkoutSession`, so the tab bar stayed hidden. **Fix (client):** `GoFitMobile/src/screens/library/WorkoutSessionScreen.tsx` — added `isLeavingWorkoutRef` and early-return in `beforeRemove` when leaving or completing; extracted `navigateFromPlanSessionToWorkoutsMain()` (reset Library to `LibraryMain`, then parent `navigate('Workouts', { screen: 'WorkoutsMain', initial: false })`); set the ref before `dispatch(e.data.action)` for non-Plan exits; pre-start Plan back uses the same helper. `GoFitMobile/src/screens/plan/MyWorkouts.tsx` — plan start dispatch to `Library -> WorkoutSession` includes `initial: false`. **CustomTabBar** intentionally unchanged. **Calendar sync:** Finishing a session only updated `workout_sessions`, not `workout_plans`, so the calendar stayed “planned.” **Fix:** After successful `completed_at` write in `finishWorkout`, if `route.params.planId` is set, call `updateWorkoutPlanStatus(planId, 'completed', sessionId)` from `GoFitMobile/src/services/workoutPlans.ts` and `useWorkoutPlansStore.getState().fetch()`; log plan/fetch errors without blocking the success path. **Types:** `planId?: string` on `WorkoutSession` (Library + Profile stacks) and `WorkoutDetail` in `GoFitMobile/src/types/index.ts`. **Threading:** `MyWorkouts` already passed `planId` on direct plan start; it now passes `planId` into `WorkoutDetail` for plan rows (card + DETAILS); `WorkoutDetailScreen` forwards `planId` and `returnTo` into `WorkoutSession` from `startWorkoutForDay`. **Gap:** Resuming a session from home/library without `planId` still does not mark a plan row (would need session↔plan linkage later).
...
### Mobile plan / calendar workout flow (shipped 2026-04-24)

Plan-started sessions: exit navigation no longer self-blocks `beforeRemove`; Library stack resets cleanly; Workouts tab opens with `WorkoutsMain` and `initial: false` where specified. Completing a session updates the linked `workout_plans` row (`status: completed`, `session_id`) and refreshes the plans store so **My Workouts** / **CalendarView** reflect completion. Full file list and edge cases: **Living log → Mobile plan-started workout exit + calendar plan completion sync (2026-04-24)**.

### Client marketplace / Elite Trainers (shipped 2026-04-24)

**RLS:** Any `coach_profiles` row with `status = 'approved'` is readable for marketplace listing (migration + MCP apply on linked Supabase project). **Names/avatars:** Not from a client-side `user_profiles` join — use RPCs `list_approved_coaches_for_marketplace` and `get_coach_profile_identity` (SECURITY DEFINER + `auth.users` fallbacks); migration also ensures `user_profiles.display_name` / `profile_picture_url` exist and backfills names. **App:** `marketplace.ts` RPC + in-memory filters; home `TopTrainers` refetch on focus; pull-to-refresh; i18n for unrated coaches. See **Living log → Marketplace Elite Trainers** and **Marketplace coach display names & profile photos** (2026-04-24).
...
- **DB + storage (2026-04-24):** See migration `database/migrations/create_progress_photos_v1.sql` and **Living log → Part 5 - Progress Photos database v1**. Store files under `progress-photos` with object key prefix `{auth.uid()}/` to satisfy storage RLS.
- **Mobile (2026-04-27):** `GoFitMobile/src/services/progressPhotos.ts`, `GoFitMobile/src/screens/progress/ProgressPhotosScreen.tsx`, `GoFitMobile/src/screens/progress/ProgressPhotoDetailScreen.tsx`, Progress stack routes, and a compact `WorkoutStatisticsScreen` entry card. Uses private signed URLs, category filters, camera/library upload, date/category/note metadata, detail view, and own-photo delete.

**Deferred:** side-by-side comparison, body-measurement scan linking, and server-side photo insights.

### Part 6: Wearable Integration (Apple Health / Google Fit)

**Shipped v1:**

- **DB (2026-04-27):** Migration `database/migrations/create_health_data_wearables_v1.sql` — `public.health_data` with `UNIQUE (user_id, date, source)`, `steps` / `active_calories` non-negative, `source` in (`health_connect`, `manual`), RLS own-row CRUD, indexes `(user_id, date desc)` and `(user_id, source, date desc)`, `set_health_data_updated_at` → `handle_updated_at()`. Applied to `rdozeaacwaisgkpxjycn` via MCP (`create_health_data_wearables_v1`).
- **Android mobile (2026-04-27):** Health Connect permissions and aggregate sync for daily steps + active calories, Home widget, Progress 7-day trend, and Profile Health Sync screen. Requires a custom Android dev build; not available in Expo Go.

**Deferred:** iOS HealthKit, background sync, sleep/heart/distance metrics, wearable vendor accounts, and admin/BI health analytics.
